...
async function serveStaticAsset(request, env) {
  ...
  const filePath = resolveAssetFilePath(path);

  if (env.ASSETS) {
    const assetPath = assetFetchPath(filePath);
    const extensionlessHtml = filePath.endsWith(".html") ? filePath.slice(0, -5) : filePath;
    const isAliased = Boolean(ASSET_FILE_ALIASES[path]);
    const useOriginalRequest = !isAliased && (
      url.pathname === "/" ||
      url.pathname === path ||
      url.pathname === assetPath ||
      url.pathname === extensionlessHtml
    );
    const assetRequest = useOriginalRequest
      ? request
      : new Request(new URL(assetPath, url.origin), request);
    return withStaticAssetHeaders(await env.ASSETS.fetch(assetRequest), path);
  }

  if (!env.VIBEFIX_KV) return fetchGitHubAsset(filePath);

  const value = await env.VIBEFIX_KV.get(`asset:${filePath}`, { type: "arrayBuffer" });

  if (!value) return fetchGitHubAsset(filePath);
...
const ASSET_FILE_ALIASES = {
  "/styles.css": "/assets/css/styles.css",
  "/site.js": "/assets/js/site.js",
  "/scanner-prompts.js": "/assets/js/scanner-prompts.js",
  "/dashboard.js": "/assets/js/dashboard.js",
  "/og-image.svg": "/assets/images/og-image.svg",
  "/free-30-minute-recovery-checklist.pdf": "/assets/downloads/free-30-minute-recovery-checklist.pdf",
  "/intake.html": "/pages/intake.html",
  "/intake-submitted.html": "/pages/intake-submitted.html",
  "/payment.html": "/pages/payment.html",
  "/payment-success.html": "/pages/payment-success.html",
  "/prompt-scope-checker.html": "/tools/prompt-scope-checker.html",
  "/rollback-calculator.html": "/tools/rollback-calculator.html",
  "/auth-broke-vibe-coded-app.html": "/guides/auth-broke-vibe-coded-app.html",
  "/bolt-app-works-in-preview-not-production.html": "/guides/bolt-app-works-in-preview-not-production.html",
  "/break-patterns.html": "/guides/break-patterns.html",
  "/cursor-broke-my-code.html": "/guides/cursor-broke-my-code.html",
  "/glossary.html": "/guides/glossary.html",
  "/lovable-app-broke-after-update.html": "/guides/lovable-app-broke-after-update.html",
  "/replit-deployment-failing.html": "/guides/replit-deployment-failing.html",
...
function resolveAssetFilePath(path) {
  return ASSET_FILE_ALIASES[path] || path;
}

function assetFetchPath(path) {
...
function normalizePromptKey(value) {
  ...
  const aliases = {
    ...
    authbroke: "authBroke",
    authloginsignup: "authBroke",
    databasenotloading: "databaseNotLoading",
    databasedatanotloading: "databaseNotLoading",
    dashboarddata: "databaseNotLoading",
    previewvsproduction: "previewVsProduction",
    previewworksbutproductionbroken: "previewVsProduction",
    envconfig: "previewVsProduction",
    fixbreakloop: "fixBreakLoop",
    aichangedunrelatedfiles: "fixBreakLoop",
    deployfailed: "deployFailed",
    deployment: "deployFailed",
    stripebroke: "stripeBroke",
    paymentcheckout: "stripeBroke",
    featurebrokeoldfeature: "featureBrokeOldFeature",
    uilayout: "featureBrokeOldFeature",
    permissions: "authBroke",
    apiintegration: "databaseNotLoading",
    fileupload: "databaseNotLoading",
    email: "other",
    routing: "other",
    blankscreen: "other"
  };
