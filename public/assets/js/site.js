...
function initHeroLiveAnimation() {
  const status = document.querySelector("#hero-live-status");
  if (!status) return;

  const messages = [
    "Reading symptom stream…",
    "Checking prompt-again risk…",
    "Mapping no-touch zones…",
    "Comparing preview vs production…",
    "Tracing likely break layer…",
    "Building case file preview…"
  ];

  if (window.matchMedia?.("(prefers-reduced-motion: reduce)")?.matches) {
    status.textContent = "Diagnostic scanner ready";
    return;
  }
...
function buildCaseFile(payload) {
  ...
  const safePrompt = typeof buildUniqueScannerPrompt === "function"
    ? buildUniqueScannerPrompt(payload, draftResult)
    : "";
  ...
  return {
    ...
    loopDetected: draftResult.loopDetected,
...
function buildToolPrompt(payload, layer, noTouchZones, tool) {
  if (typeof buildUniqueScannerPrompt !== "function") return "";
  return buildUniqueScannerPrompt(payload, { layer, noTouchZones, promptRisk: scannerState.result?.promptRisk, confidence: scannerState.result?.confidence, rollbackDirection: scannerState.result?.rollbackDirection, loopDetected: scannerState.result?.loopDetected }, tool);
}
...
function renderCaseFile(payload, result) {
  ...
  setText("#safe-first-prompt", result.safePrompt || "Run the scanner again to generate a tool-specific repair prompt.");
...
## Repair Prompt
${result.safePrompt || "Run the scanner to generate a tool-specific prompt."}
